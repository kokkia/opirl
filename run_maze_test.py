...
if __name__ == "__main__":
    ...
    # (expert_states, expert_actions, expert_next_states, expert_dones, expert_logps) = data_utils.add_absorbing_states(
    #     expert_states, expert_actions, expert_next_states, expert_dones, expert_logps, env
    # )
    ...
    for j in range(10):
        ...
        for i in range(max_steps):
            mean_action, _, logp = policy.actor.call(np.array([obs]))
            action = mean_action[0].numpy()
            # action = (action + np.random.normal(0, 0.1, size=action.shape)).clip(-1, 1)
            next_obs, reward, done, diagnostics = env.step(action)
            env.render()
            reward_log = reward
            print("reward = ", reward)
            time.sleep(0.1)

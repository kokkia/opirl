        ...
        self.critic1 = tf.keras.Sequential(
            [
                tf.keras.layers.Dense(
                    256, input_shape=(state_dim + action_dim,), activation=tf.nn.relu, kernel_initializer="orthogonal"
                ),
                tf.keras.layers.Dense(256, activation=tf.nn.relu, kernel_initializer="orthogonal"),
...
class Policy(OffPolicyAgent):
    ...
    def critic_mix(self, s, a):
        ...
        return (
            q1 * 0.05 + target_q * 0.95,
            q2 * 0.05 + target_q * 0.95,
        )

    def fit_critic(self, states, actions, next_states, rewards, masks, init_states):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            critic_loss1 = tf.reduce_mean(self.f(target_q1 - q1) + (1 - self.discount) * init_q1 * self.algae_alpha)

            critic_loss2 = tf.reduce_mean(self.f(target_q2 - q2) + (1 - self.discount) * init_q2 * self.algae_alpha)
    ...
    def fit_actor(self, states, actions, next_states, rewards, masks, target_entropy, init_states):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            alpha_loss = tf.reduce_mean(self.alpha * (-next_log_probs - target_entropy))
    ...
    def train(self, states, actions, next_states, rewards, masks, init_states, target_entropy=0, actor_update_freq=2):
        ...
        if tf.equal(self.critic_optimizer.iterations % actor_update_freq, 0):
            actor_loss, alpha_loss, entropy, bc_loss = self.fit_actor(
                states, actions, next_states, rewards, masks, target_entropy, init_states
            )

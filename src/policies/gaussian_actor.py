...
from src.misc.keras_utils import load_keras_model
...
class Actor(tf.keras.Model):
    ...
    def __init__(self, state_dim, action_dim, loaded_actor=None, use_multivariate=False):
        """Creates an actor.
            ...
            self.trunk = tf.keras.Sequential(
                [
                    tf.keras.layers.Dense(
                        256, input_shape=(state_dim,), activation=tf.nn.relu, kernel_initializer="orthogonal"
                    ),
    ...
    def get_dist_and_mode(self, states):
        ...
        if self._use_multivariate:
            ...
            dist = ds.TransformedDistribution(
                ds.Sample(ds.Normal(tf.zeros(mu.shape[:-1]), 1.0, validate_args=True), sample_shape=mu.shape[-1:]),
                tfp.bijectors.Chain(
                    [
    ...
    def load_model(self, path):
        self.trunk = load_keras_model(path)

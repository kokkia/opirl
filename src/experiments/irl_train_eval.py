...
class IRLTrainer(BaseTrainer):
    def __init__(
        ...
        self.replay_buffer_iter = iter(self.replay_buffer.as_dataset(sample_batch_size=args.sample_batch_size))

        self.policy_replay_buffer = get_replay_buffer(env, size=buffer_size)
        self.policy_replay_buffer_iter = iter(
            self.policy_replay_buffer.as_dataset(sample_batch_size=args.sample_batch_size)
        )

        self._use_init_states = use_init_states
        if use_init_states:
            init_spec = tensor_spec.TensorSpec([env.observation_space.shape[0]], tf.float32, "observation")
            self.init_replay_buffer = get_replay_buffer(env, size=args.max_timesteps, spec=init_spec)
            self.init_replay_buffer_iter = iter(
                self.init_replay_buffer.as_dataset(sample_batch_size=self._sample_batch_size)
            )
        ...
        if not no_expert:
            ...
            expert_dataset = tf.data.Dataset.from_tensor_slices((expert_states, expert_actions, expert_next_states))

            self.expert_dataset = (
                expert_dataset.repeat()
                .shuffle(expert_states.shape[0])
                .batch(self._sample_batch_size, drop_remainder=True)
            )
        ...
        self.video_recorder = VideoRecorder(self._output_dir if self._save_test_movie else None)
    ...
    def __call__(self):
        ...
        while total_timesteps <= self._max_timesteps:
            if total_timesteps % self._eval_interval == 0:
                ...
                average_returns, evaluation_timesteps = self.evaluate(total_timesteps)

                eval_returns.append(average_returns)

                self.logger.log("eval/average_test_return", average_returns)
                self.logger.log("eval/average_test_episode_length", evaluation_timesteps)
                self.logger.dump(total_timesteps, dump_to_csv=True, dump_eval=True)
                logging.info("Eval: ave returns=%f, ave episode length=%f", average_returns, evaluation_timesteps)
                if self._save_model:
                    save_keras_model(
                        self.imitator.rew_net, os.path.join(self._model_dir, "rew_net" + str(total_timesteps))
                    )
                    save_keras_model(
                        self.policy.actor.trunk, os.path.join(self._model_dir, "actor_net" + str(total_timesteps))
                    )
            if done:
            ...
            if total_timesteps < self._num_random_actions:
                ...
                logp = self.policy.get_logp(np.array([obs]), np.array([action]))
            else:
            ...
            add_samples_to_replay_buffer(self.replay_buffer, obs, action, next_obs, logp)
            add_samples_to_replay_buffer(self.policy_replay_buffer, obs, action, next_obs, logp)
            if done and not truncated_done:
                ...
                for abs_i in range(self._absorbing_per_episode):
                    if abs_i + episode_timesteps < self.env._max_episode_steps:  # pylint: disable=protected-access
                        ...
                        action = self.env.action_space.sample()
                        next_obs = self.env.get_absorbing_state()
                        logp = self.policy.actor.get_log_prob(np.array([obs]), np.array([action]))
    ...
    def evaluate(self, total_timesteps, num_episodes=20):
        ...
        for i in range(num_episodes):
            ...
            while not done:
                action, _, _ = self.policy.actor(np.array([state]), training=False)
            ...
            if self._save_test_movie and total_timesteps in self._save_movie_indices:
                ...
                self.video_recorder.save(f"{total_timesteps}.mp4", logger=self.logger, step=total_timesteps)
    ...
    def _train_imitator(self):
        for _ in range(self._imitator_updates_per_step):
            return_dict = self.imitator.update(**self._get_imitator_train_kwargs())

            self.logger.log("train/disc_loss", return_dict["train/gail_loss"])
        with self.airl_writer.as_default():
            tf.summary.scalar(
                "gail_classification_loss", return_dict["train/gail_classification_loss"], self.total_steps
            )
    ...
    def _train_policy(self):
        for _ in range(self._policy_updates_per_step):
            if self._algo == "bc":
                expert_states, expert_actions, expert_next_states = next(self.expert_dataset_iter)
                return_dict = self.policy.train_bc(expert_states, expert_actions)
            else:
                kwargs = self._get_policy_train_kwargs()
                return_dict = self.policy.train(**kwargs)
                return_dict["train/imitator_reward"] = np.mean(kwargs["rewards"])
    ...
    def _get_policy_train_kwargs(self):
        states, actions, next_states, _, masks, logp = next(self.replay_buffer_iter)[0]

        rewards = self.imitator.predict_reward(states, actions, next_states, logp)
        kwargs = {
            "states": states,
            "actions": actions,
    ...
    def _get_imitator_train_kwargs(self):
        expert_states, expert_actions, expert_next_states = next(self.expert_dataset_iter)
        policy_states, policy_actions, policy_next_states, _, _, _ = next(self.policy_replay_buffer_iter)[0]
